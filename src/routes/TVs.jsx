...
function Movies() {
  ...
  const getTrendingMovies = async () => {
    const json = await (
      await fetch(
        `https://api.themoviedb.org/3/trending/tv/day?api_key=056adb2993b61aff8516b32900fa97e7&page=${currentPage}`
      )
    ).json();
  ...
  const getMovies = async () => {
    const json = await (
      await fetch(
        `https://api.themoviedb.org/3/search/tv?api_key=056adb2993b61aff8516b32900fa97e7&query=${keySearch}&page=${currentPage}`
      )
    ).json();
    setTotalPage(json.total_pages);
    if (movies === null) {
  ...
  function search() {
    ...
    {
      alert('Please enter a text search term');
      textSearch.current.focus();
    } else {
      setMovies(null)
