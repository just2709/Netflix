          ...
          <div className='max-w-[1200px] mx-auto px-2 w-full pt-[150px] sm:grid grid-cols-3 gap-5'>
            ...
            <div className='col-span-2'>
              ...
              {movie.genres.map((item, index) => (
                <button key={index} className='px-2 rounded-full border-2 mr-2 text-xl'>
                  {item.name}
                </button>
              ))}
              <h1 className='mt-5'><StarRating className='block w-full' rate={movie.vote_average} /></h1>
            ...
            {video && (
              video.map((item, index) => (
                <Video className='col-span-3' key={index} keyVideo={item.key} name={item.name} />
              )
            ))}
